...
exports.getCompletionStatus = exports.confirmProjectCompletion = exports.requestProjectCompletion = exports.uploadProjectFiles = exports.deleteProject = exports.updateProject = exports.createProject = exports.getProjectById = exports.getAllProjects = void 0;
...
const getAllProjects = async (req, res) => {
    try {
        ...
        if (presupuestoMin || presupuestoMax) {
            where.presupuesto = {};
            if (presupuestoMin)
                where.presupuesto.gte = Number.parseFloat(String(presupuestoMin));
            if (presupuestoMax)
                where.presupuesto.lte = Number.parseFloat(String(presupuestoMax));
        }
...
const createProject = async (req, res) => {
    try {
        const { nombre, descripcion, tecnologias, tipoProyecto, presupuesto, presupuestoTipo, duracionEstimada, ubicacion, usuarioCreadorId, fechaLimite, tamanoEquipo, adjuntos, datosAdicionales, } = req.body;
...
const updateProject = async (req, res) => {
    try {
        const { id } = req.params;
        const { titulo, descripcion, tecnologias, estado, tipoProyecto, presupuesto, presupuestoTipo, duracionEstimada, ubicacion, fechaLimite, tamanoEquipo, adjuntos, datosAdicionales, } = req.body;
        const updateData = {};
        if (titulo)
            updateData.nombre = titulo;
        if (descripcion)
            updateData.descripcion = descripcion;
        if (tecnologias)
            updateData.tecnologias = Array.isArray(tecnologias) ? JSON.stringify(tecnologias) : tecnologias;
        if (estado)
            updateData.estado = estado;
        if (tipoProyecto)
            updateData.tipoProyecto = tipoProyecto;
        if (presupuesto)
            updateData.presupuesto = Number.parseFloat(presupuesto);
        if (presupuestoTipo)
            updateData.presupuestoTipo = presupuestoTipo;
        if (duracionEstimada)
        ...
        if (tamanoEquipo)
            updateData.tamanoEquipo = tamanoEquipo;
        if (adjuntos)
            updateData.adjuntos = Array.isArray(adjuntos) ? JSON.stringify(adjuntos) : adjuntos;
        if (datosAdicionales)
            updateData.datosAdicionales = datosAdicionales;
...
const uploadProjectFiles = async (req, res) => {
    try {
        ...
        const storedPaths = files.map(f => `/${f.path.replace(/\\/g, '/').replace(/^uploads\//, 'uploads/')}`);
        // Fetch existing project
        const proyecto = await prisma.proyecto.findUnique({ where: { id: Number.parseInt(id) } });
        if (!proyecto)
            return res.status(404).json({ error: 'Proyecto no encontrado' });
        const existing = proyecto.adjuntos ? (Array.isArray(proyecto.adjuntos) ? proyecto.adjuntos : JSON.parse(proyecto.adjuntos)) : [];
...
exports.requestProjectCompletion = requestProjectCompletion;
...
exports.confirmProjectCompletion = confirmProjectCompletion;
