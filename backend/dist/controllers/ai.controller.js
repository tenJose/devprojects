...
exports.generateQuestions = exports.generateFromPrompt = void 0;
...
Devuelve este objeto JSON con las claves (obligatorias cuando apliquen):
{
  ...
  "teamSize": "string"
        ...
        try {
            ...
            // Helpers
            const isMobileKeyword = (text) => /móvil|movil|aplicación móvil|app|ios|android|flutter|react native|react-native|reactnative/.test(text);
            // Title
            ...
            if (!Array.isArray(normalized.title_suggestions))
                normalized.title_suggestions = [];
            // If no title but we have suggestions, pick first as title
            if (!normalized.title && normalized.title_suggestions.length)
                normalized.title = normalized.title_suggestions[0];
            // Description
            ...
            if (typeof normalized.techStack === 'string') {
                try {
                    normalized.techStack = JSON.parse(normalized.techStack);
                }
                catch {
                    normalized.techStack = [normalized.techStack];
                }
            }
            if (!Array.isArray(normalized.techStack))
            ...
            if (!normalized.platform) {
                if (isMobileKeyword(lowerPrompt)) {
                    ...
                    if (/ios/.test(lowerPrompt))
                        normalized.platform = 'iOS';
                    else if (/android/.test(lowerPrompt))
                        normalized.platform = 'Android';
                    else
                        normalized.platform = 'Cross-platform';
                }
                else {
            ...
            if (!normalized.techStack || normalized.techStack.length === 0) {
                if (String(normalized.type).toLowerCase() === 'mobile')
                    normalized.techStack = ['React Native', 'Firebase'];
                else if (String(normalized.type).toLowerCase().includes('full'))
                    normalized.techStack = ['React', 'Node.js', 'MySQL'];
                else
                    normalized.techStack = ['React'];
            }
            // Populate simple budget/duration/teamSize defaults if not provided
            if (!normalized.duration) {
                if (String(normalized.type).toLowerCase() === 'mobile')
                    normalized.duration = '2-4 meses';
                else if (String(normalized.type).toLowerCase().includes('full'))
                    normalized.duration = '3-6 meses';
                else
                    normalized.duration = '1-3 meses';
            }
            if (!normalized.budget) {
                if (String(normalized.type).toLowerCase() === 'mobile')
                    normalized.budget = 15000;
                else if (String(normalized.type).toLowerCase().includes('full'))
                    normalized.budget = 30000;
                else
                    normalized.budget = 8000;
            }
            if (!normalized.teamSize) {
                if (String(normalized.type).toLowerCase() === 'mobile')
                    normalized.teamSize = '2-4';
                else if (String(normalized.type).toLowerCase().includes('full'))
                    normalized.teamSize = '3-6';
                else
                    normalized.teamSize = '1-2';
            }
...
exports.generateFromPrompt = generateFromPrompt;
const generateQuestions = async (req, res) => {
    try {
        ...
        const historyText = Array.isArray(history) && history.length ?
            history.map((h, i) => `Q${i}: ${h.question}\nA${i}: ${h.answer}`).join('\n') : '';
...
Or if ready to generate:
{
  ...
  "done": true
        ...
        try {
            ...
            const questions = Array.isArray(parsed.questions) ? parsed.questions.filter((q) => q && typeof q === 'string').slice(0, 1) : [];
